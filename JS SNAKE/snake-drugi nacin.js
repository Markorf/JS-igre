...
const snake = {
    ...
    moveSnake(newXPos = 0, newYPos = 0) {
        ...
        this.interval = setInterval(() => {            
            this.snakeArr[0].snakeXPos+=newXPos;
            this.snakeArr[0].snakeYPos+=newYPos;
            this.checkForEat();
            
            if (this.snakeArr.length > 1) {
                for (let i = 1; i < this.snakeArr.length; i++) {
                    this.snakeArr[i].snakeXPos = this.odlPosArr[i-1].oldXPos;
                    this.snakeArr[i].snakeYPos = this.odlPosArr[i-1].oldYPos;
                }
            }
            for (let i = 0; i < this.snakeArr.length; i++) {                                
                this.odlPosArr[i].oldXPos = this.snakeArr[i].snakeXPos;
                this.odlPosArr[i].oldYPos = this.snakeArr[i].snakeYPos;
            }
            this.repaint();        
            this.drawSnake();
        }, 50);
    ...
    checkForEat() {
        if (this.snakeArr[0].snakeXPos == food.xPos && this.snakeArr[0].snakeYPos == food.yPos) {
            ...
            this.snakeArr.push({snakeXPos: 0, snakeYPos:0});
            this.odlPosArr.push({}); // mora da ne bi bio undefined error
        } else {
            //
        }
    },
